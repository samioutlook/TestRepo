...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:variable name="testDetailsFilter" select="if ($testNgXslt.testDetailsFilter) then
$testNgXslt.testDetailsFilter else 'FAIL,PASS,SKIP'"/>
...
testng:getVariableSafe($testNgXslt.chartScaleFactor, 1))"/>
 ...
 <![CDATA[
 body { font-family: Arial, sans-serif; font-size: 12px; padding: 10px; margin: 0px; backgroundcolor:
white; }
 a, a:hover, a:active, a:visited { color: navy; }
 .suiteMenuHeader { margin-top: 10px; }
 .suiteMenuHeader td { padding: 5px; background-color: #e0e0e0; font-size: 12px; width: 100%;
vertical-align: top; }
 .suiteStatusPass, .suiteStatusFail { padding-right: 20px; width: 20px; height: 20px; margin: 2px 4px
2px 2px; display: inline; }
 ...
 .testCaseFail, .testCasePass, .testCaseSkip { padding-right: 15px; width: 15px; height: 15px;
margin: 2px 4px 2px 2px; display: inline; }
 ...
 .testMethodStatusFAIL a, .testMethodStatusPASS a, .testMethodStatusSKIP a { color:navy; textdecoration:
none; cursor: pointer; }
 .testMethodStatusFAIL td, .testMethodStatusPASS td, .testMethodStatusSKIP td { padding: 3px; }
 .testMethodDetails, .testMethodDetailsVisible { padding: 5px; background-color: #f5f5f5; margin:
1px; }
 ...
 .testMethodsTable .detailsBox { padding: 0; background-color: white; border-style: none; height:
0px; }
 ...
 document.getElementById("testMethodsByStatus").style["display"] = checkbox.checked ?
"none" : "block";
 document.getElementById("testMethodsByClass").style["display"] = checkbox.checked ?
"block" : "none";
 }
 ...
 filterFail.checked = filterPass.checked = filterSkip.checked = filterConf.checked =
filterAll.checked;
 ...
 if ((filterFail.checked == filterPass.checked) && (filterPass.checked == filterSkip.checked) &&
(filterSkip.checked == filterConf.checked)) {
 ...
 var text = document.createTextNode('SVG Pie Charts are not available. Please install a SVG
viewer for your browser.');
 ...
 <xsl:value-of
 select="testng:safeFileName(concat($testCaseElement/../@name, '_',
$testCaseElement/@name, '.html'))"/>
 </xsl:function>
 ...
 <xsl:value-of select="if ($testMethodElement/@is-config) then concat($statusClass, '
testMethodStatusCONF') else $statusClass"/>
 </xsl:function>
 <xsl:function name="testng:suiteMethodsCount">
 <xsl:param name="testCasesElements"/>
 <xsl:param name="state"/>
 <xsl:value-of
 select="if ($state = '*') then count($testCasesElements/class/test-method[not(@is-config)])
 ...
 <xsl:param name="state"/>
 <xsl:value-of
 select="if ($state = '*') then count($testCaseElement/class/test-method[not(@is-config)]) else
count($testCaseElement/class/test-method[(@status=$state) and (not(@is-config))])"/>
 </xsl:function>
 <xsl:function name="testng:suiteStateClass">
 <xsl:param name="testCaseElements"/>
 <xsl:value-of select="if (count($testCaseElements/class/test-method[(@status='FAIL') and (not(@isconfig))])
> 0) then 'suiteStatusFail' else 'suiteStatusPass'"/>
 </xsl:function>
...
width="{$chartWidth}" height="{$chartHeight}"
 ...
 <style type="text/css">
 <![CDATA[
 .axistitle { font-weight:bold; font-size:24px; font-family:Arial;
text-anchor:middle; }
 .xgrid, .ygrid, .legendtext { font-weight:normal; font-size:24px;
font-family:Arial; }
 .xgrid {text-anchor:middle;}
 .ygrid {text-anchor:end;}
 .gridline { stroke:black; stroke-width:1; }
 .values { fill:black; stroke:none; text-anchor:middle; fontsize:12px;
font-weight:bold; }
 ]]>
 </style>
 </defs>
 ...
 <xsl:variable name="failedCount" select="testng:suiteMethodsCount($testCaseElements,
'FAIL')"/>
 <xsl:variable name="passedCount" select="testng:suiteMethodsCount($testCaseElements,
'PASS')"/>
 <xsl:variable name="skippedCount" select="testng:suiteMethodsCount($testCaseElements,
'SKIP')"/>
...
'###%')"/>
 ...
 <xsl:variable name="failedY_text" select="(-1 * ($radius div 2) *
math:sin($failedAngle_text))"/>
...
height="20"/>
 ...
 <g style="fill:red"
 transform="rotate(-{$failedRotation}) translate({round($failedX_text div 4)},
{round($failedY_text div 4)})">
 <path d="M 0 0 h {$radius} A {$radius},{$radius} 0,{$failedArc},0 {$failedX},{$failedY}
z"/>
 </g>
 </xsl:if>
 <xsl:variable name="passRotation" select="(($failedCount + $skippedCount) div
$totalCount) * 360"/>
...
select="$chartHeight"/>);
 ...
 <xsl:value-of
 select="if (testng:testCaseMethodsCount(., '*') > 0) then formatnumber(testng:testCaseMethodsCount(.,
'PASS') div testng:testCaseMethodsCount(., '*'), '###%') else
'100%'"/>
 </td>
 ...
 <xsl:value-of
 select="format-number(testng:suiteMethodsCount($testCaseElements,
'PASS') div testng:suiteMethodsCount($testCaseElements, '*'), '###%')"/>
 </td>
 </tr>
 </table>
 ...
 <xsl:param name="suiteElement"/>
 <xsl:variable name="testCaseElements" select="if (test/@url) then document(test/@url)/test else
test"/>
 <xsl:result-document href="{testng:absolutePath(testng:suiteContentFileName($suiteElement))}"
format="xhtml">
 <html>
 <xsl:call-template name="htmlHead"/>
 <body>
 ...
 <xsl:with-param name="failedMethods" select="$testCaseElements/class/testmethod[@status='FAIL']"/>
 <xsl:with-param name="passedMethods" select="$testCaseElements/class/testmethod[@status='PASS']"/>
 <xsl:with-param name="skipedMethods" select="$testCaseElements/class/testmethod[@status='SKIP']"/>
 </xsl:call-template>
 <xsl:call-template name="powered-by"/>
 </body>
 </html>
...
onclick="switchTestMethodsView(this)">
 ...
 <input id="methodsFilter_ALL" type="checkbox" onclick="testMethodsFilterChanged(this,
'ALL')">
 <xsl:if test="testng:isFilterSelected('FAIL') = 'true' and testng:isFilterSelected('PASS') = 'true'
and testng:isFilterSelected('SKIP') = 'true' and testng:isFilterSelected('CONF') = 'true'">
 <xsl:attribute name="checked" select="true"/>
 </xsl:if>
 </input>
 All
 ...
 <input id="methodsFilter_FAIL" type="checkbox" onclick="testMethodsFilterChanged(this,
'FAIL')">
 ...
 <input id="methodsFilter_PASS" type="checkbox" onclick="testMethodsFilterChanged(this,
'PASS')">
 ...
 <input id="methodsFilter_SKIP" type="checkbox" onclick="testMethodsFilterChanged(this,
'SKIP')">
 ...
 <input id="methodsFilter_CONF" type="checkbox" onclick="testMethodsFilterChanged(this,
'CONF')">
 ...
 <xsl:sort order="ascending" select="if (compare($sortByStartTime, 'true') = 0) then @started-at
else ''"/>
 <xsl:variable name="methodId" select="concat(../@name, '_', @name, '_', $category, '_',
@status, position())"/>
...
testng:trim(testng:concatParams(./params/param)), ')')"/>
 </a>
 </td>
 <td nowrap="true">
 ...
 <xsl:sort order="ascending" select="if (compare($testNgXslt.sortTestCaseLinks, 'true') = 0) then
@name else ''"/>
 <div class="testCaseLink"
 onclick="javscript:selectTestCaseLink(this);
parent.content.location='{testng:testCaseContentFileName(.)}'">
 <div class="{if (count(./class/test-method[@status='FAIL']) > 0)
 then 'testCaseFail'
 else if ((count(./class/test-method[@status='FAIL']) = 0) and (count(./class/testmethod[@status='PASS'])
> 0))
 then 'testCasePass'
 else 'testCaseSkip'}">
 </div>
 ...
 <xsl:param name="testCases"/>
 <xsl:for-each select="$testCases">
 <xsl:result-document href="{testng:absolutePath(testng:testCaseContentFileName(.))}"
format="xhtml">
 <html>
 <xsl:call-template name="htmlHead"/>
 <body>
 ...
 <xsl:call-template name="percentageOverview">
 <xsl:with-param name="failedCount" select="testng:testCaseMethodsCount(.,
'FAIL')"/>
 <xsl:with-param name="passedCount" select="testng:testCaseMethodsCount(.,
'PASS')"/>
 <xsl:with-param name="skippedCount"
 ...
 <xsl:with-param name="passedMethods" select="./class/testmethod[@status='PASS']"/>
 ...
 <div style="width: 50px;">
 <xsl:value-of
 select="if ($totalCount > 0) then format-number($passedCount div $totalCount, '###%')
else '100%'"/>
 </div>
 </td>
 <xsl:if test="compare($testNgXslt.showRuntimeTotals, 'true') = 0">
